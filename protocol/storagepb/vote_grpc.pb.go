...
const (
	...
	Vote_VotesSubscribe_FullMethodName = "/storagepb.Vote/VotesSubscribe"
)
...
type VoteClient interface {
	...
	VotesSubscribe(ctx context.Context, in *VotesSubscribeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[VoteInfo], error)
}
...
func (c *voteClient) VotesSubscribe(ctx context.Context, in *VotesSubscribeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[VoteInfo], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Vote_ServiceDesc.Streams[0], Vote_VotesSubscribe_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[VotesSubscribeRequest, VoteInfo]{ClientStream: stream}
...
type Vote_VotesSubscribeClient = grpc.ServerStreamingClient[VoteInfo]
...
type VoteServer interface {
	...
	VotesSubscribe(*VotesSubscribeRequest, grpc.ServerStreamingServer[VoteInfo]) error
...
func (UnimplementedVoteServer) GetVotes(context.Context, *VotesFilterRequest) (*VotesFilterResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetVotes not implemented")
}
...
func (UnimplementedVoteServer) GetDaosVotedIn(context.Context, *DaosVotedInRequest) (*DaosVotedInResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetDaosVotedIn not implemented")
}
func (UnimplementedVoteServer) VotesSubscribe(*VotesSubscribeRequest, grpc.ServerStreamingServer[VoteInfo]) error {
	return status.Error(codes.Unimplemented, "method VotesSubscribe not implemented")
}
...
func RegisterVoteServer(s grpc.ServiceRegistrar, srv VoteServer) {
	// If the following call panics, it indicates UnimplementedVoteServer was
...
func _Vote_VotesSubscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(VotesSubscribeRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(VoteServer).VotesSubscribe(m, &grpc.GenericServerStream[VotesSubscribeRequest, VoteInfo]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Vote_VotesSubscribeServer = grpc.ServerStreamingServer[VoteInfo]
...
var Vote_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "VotesSubscribe",
			Handler:       _Vote_VotesSubscribe_Handler,
			ServerStreams: true,
		},
	},
